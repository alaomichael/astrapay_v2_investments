export const filterArrayByCriteria = async function filterArrayByCriteria(data, timestamp, limit) {
    const targetTimestampUTC = new Date(timestamp);

    const filteredData = data.filter((item) => {
        const attempts = Number(item.attempts);
        const maxAttempts = Number(item.max_attempts);
        const lastAttemptAt = new Date(item.last_attempt_at);
        const createdAtDateUTC = new Date(item.created_at);

        let retryPeriodUTC 
        if(lastAttemptAt){
            retryPeriodUTC = new Date(lastAttemptAt.getTime() + (Number(item.retry_period) * 60 * 60 * 1000)); // In hours format
        }else{
    ...
    filteredData.sort((a, b) => {
        if (!a.last_attempt_at && b.last_attempt_at) {
            return -1;
        } else if (a.last_attempt_at && !b.last_attempt_at) {
            return 1;
        } else if (!a.last_attempt_at && !b.last_attempt_at) {
            return 0;
        } else {
            return new Date(b.last_attempt_at).getTime() - new Date(a.last_attempt_at).getTime();
        }
    });
...
const timestamp = "2024-01-10T00:00:00.0+01:00"; // Replace with your timestamp
