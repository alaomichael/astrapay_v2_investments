...
export default class InvestmentsServices {
    ...
    public async getInvestmentsSum(queryParams: any): Promise<any> {
        try {
            const queryGetter = await this.queryBuilder(queryParams);
            const counter = await Investment.query()
                .whereRaw(queryGetter.sqlQuery, queryGetter.params)
                // .sum("amount");
                .sum("total_amount_to_payout");

            // debugger
            return parseInt(counter[0].$extras.sum);
        } catch (error) {
            console.log(error);
            throw error;
        }
    ...
    public async collateAboutToBeMatureInvestment(queryParams: any): Promise<Investment[] | any> {
        ...
        try {
            ...
            let { limit, offset = 0, updatedAtFrom, updatedAtTo, payoutDateFrom, payoutDateTo, numberOfDaysFromToday, numberOfDaysBeforeToday, rfiCode } = queryParams;
            ...
            //    const settingsService = new SettingsServices();
            // const timelineService = new TimelinesServices();
            console.log("payoutDateTo line 1889 =========================")
            console.log(payoutDateTo)
            console.log("numberOfDaysFromToday line 1891 =========================")
            console.log(numberOfDaysFromToday)
            console.log("limit line 1899 =========================")
            console.log(limit)
            let responseData = await Database
                .from('investments')
                // .useTransaction(trx) // 👈
                .where('status', "active")
                .orWhere('rfi_code', rfiCode)
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 2301:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
                    continue;
                }
            }
    ...
    public async setInvestmentForMaturityById(id: string): Promise<Investment[] | any> {
        // const trx = await Database.transaction();
        try {
            let responseData = await Database
                .from('investments')
            ...
            const processInvestment = async (investment) => {
                ...
                try {
                    ...
                    // debugger
                    if (investment && investment.$original.status == "active" && investment.$original.isPayoutSuspended == false && investment.$original.isRolloverSuspended == false) {
                        // console.log('investment search data :', investment.$original)
                        let { rfiCode, duration, walletId, userId } = investment.$original;
                        // Update startDate by deducting duration
                        let record = await investmentsService.getInvestmentsByIdAndWalletIdAndUserId(investmentId, walletId, userId);
                        // send for update
                        investment.startDate = DateTime.now().minus({ days: duration }).toISO();
                        investment.payoutDate = DateTime.now().toISO();//minus({ days: duration }).toISO();
                        debugger
                        await investmentsService.updateInvestment(record, investment);
                        record = await investmentsService.getInvestmentsByIdAndWalletIdAndUserId(investmentId, walletId, userId);
                        let { startDate, investmentTypeId, } = record!;//investment.$original;
                        let timelineObject
                        let isDueForPayout = await dueForPayout(startDate, duration)
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 2547:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 1620:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 3624:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 4024:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 4578:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 5107:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 5631:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 6144:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
    ...
    public async payoutMaturedInvestment(queryParams: any, loginUserData?: any): Promise<Investment[] | any> {
        ...
        try {
            ...
            let settings;
            if (rfiCode) {
                settings = await settingsService.getSettingBySettingRfiCode(rfiCode);
                debugger
                if (!settings) {
                    throw Error(`Unable to get the Registered Financial institution with rfiCode: ${rfiCode}.`);
                };
            } else {
                settings = await settingsService.getSettings(settingQueryParams);
                debugger
                if (!settings) {
            ...
            if (settings && settings.length > 0) {
                for (let index = 0; index < settings.length; index++) {
                    ...
                    try {
                        // setting = settings.find(o => o.rfiCode === 'apmfb');// && o.identifier === payoutWalletId);

                        // debugger
                        settingsArray.push(setting);
                    } catch (error) {
                        console.log("Error line 6239:", error);
                        const timestamp = new Date().toISOString();
                        const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                        fs.appendFileSync('error.log', errorLog);
            ...
            let { payoutWalletId, } = settingsArray[0] ? settingsArray[0] : settings;
            ...
            // compare the wallet with the sum of amount to be payout
            let totalSum = await Database
                .from('investments')
                // .useTransaction(trx) // 👈
                .where('status', 'matured')
                .andWhere('request_type', 'payout_investment')
            ...
            //     // add checkedForPaymentAt
            // }
            // const settings = await settingsService.getSettings(settingQueryParams);
            // debugger
            // if (!settings) {
            //     throw Error(`Unable to get the list of Registered Financial institution.`);
            // }
            ...
            if (responseData.length > 0 && totalSum[0]!.totalAmountDueForPayout > 0) {
                // Get the current value in the payout wallet
                // debugger
                let period = 7;// LOAN_NUMBER_OF_MONTHS_OF_BALANCE_TO_FETCH ? LOAN_NUMBER_OF_MONTHS_OF_BALANCE_TO_FETCH : 3; // 90 days equals 3 months 
                console.log("period: ", period);
                startDate = startDate ? startDate : DateTime.now().minus({ days: period }).toISODate() //"2019-01-01";
                endDate = endDate ? endDate : DateTime.now().toISODate() //"2019-01-31";
                console.log("Start Date :", startDate);
                console.log("End Date :", endDate);
                console.log("Wallet Id :", payoutWalletId);
                // Get payout wallet balance
                let genericWalletBalance = await getUserWalletBalanceRecord(startDate, endDate, payoutWalletId);
                let userBalance;
                // console.log("The API Response, line 100: ", genericWallet);
                // debugger
                // if (Response.data.status === "OK") {
                if (genericWalletBalance.length > 0) {
                    // debugger
                    // userRecord = genericWalletBalance.find(o => o.walletType === 'Main Wallet' && o.identifier === payoutWalletId);
                    userBalance = (genericWalletBalance[0].finalAvailableBalance)// / 100; // convert to Naira from kobo
                    // debugger
                    if (!userBalance) return new Error(`The Balance for the User with wallet Id: ${payoutWalletId} does not exist. Please check and try again.`);
                } else {
                    debugger
                    return { status: "FAILED", message: "This user does not have a balance or wallet." };
                }
                console.log("userBalance line 6338", userBalance);
                // debugger
                //Notify admin and throw an Exception if the wallet balance is lower than the amount due for payout
                if (totalSum[0]!.totalAmountDueForPayout > userBalance) {
                    // Send Notification to Admin to fund paaout wallet
                    console.log(`There is no sufficient balance in the Investment Payout Wallet for the payment or matured investment. Please fund the Wallet with ID: ${payoutWalletId}, thank you.`);
                    ...
                    throw new AppException({ message: `There is no sufficient balance in the Investment Payout Wallet for the payment or matured investment. Please fund the Wallet with ID: ${payoutWalletId}, thank you.`, codeSt: "404" });
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 6974:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 8123:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 9348:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 10356:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 11547:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
        ...
        try {
            ...
            for (let index = 0; index < responseData.length; index++) {
                ...
                try {
                    ...
                    console.log("Error line 12815:", error);
                    const timestamp = new Date().toISOString();
                    const errorLog = `Error occurred at ${timestamp}: ${error.message}\n`;
                    fs.appendFileSync('error.log', errorLog);
                    // Continue to the next iteration of the loop instead of throwing the error
    ...
    private async queryBuilder(queryFields: any) {
        ...
        if (queryFields.rolloverType) {
            predicateExists()
            predicate = predicate + "rollover_type=?"
            params.push(queryFields.rolloverType)
        }
        ...
        if (queryFields.isRequestSent) {
            predicateExists()
            predicate = predicate + "is_request_sent=?"
            params.push(queryFields.isRequestSent)
        }

        if (queryFields.isInvestmentCreated) {
            predicateExists()
            predicate = predicate + "is_investment_created=?"
            params.push(queryFields.isInvestmentCreated)
        }

        if (queryFields.isInvestmentCompleted) {
            predicateExists()
            predicate = predicate + "is_investment_completed=?"
            params.push(queryFields.isInvestmentCompleted)
        }
        ...
        if (queryFields.isPayoutAuthorized) {
            predicateExists()
            predicate = predicate + "is_payout_authorized=?"
            params.push(queryFields.isPayoutAuthorized)
        }

        if (queryFields.investmentCompletionDate) {
            predicateExists()
            predicate = predicate + "investment_completion_date=?"
            params.push(queryFields.investmentCompletionDate)
        }

        if (queryFields.principalPayoutStatus) {
            predicateExists()
            predicate = predicate + "principal_payout_status=?"
            params.push(queryFields.principalPayoutStatus)
        }

        if (queryFields.interestPayoutStatus) {
            predicateExists()
            predicate = predicate + "interest_payout_status=?"
            params.push(queryFields.interestPayoutStatus)
        }

        if (queryFields.certificateUrl) {
            predicateExists()
            predicate = predicate + "certificate_url=?"
            params.push(queryFields.certificateUrl)
        }
