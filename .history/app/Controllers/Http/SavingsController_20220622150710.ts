...
export default class SavingsController {
  public async index({ params, request, response }: HttpContextContract) {
    ...
    // const user = await Saving.query().offset(0).limit(1)
    const user = await Saving.all()
    let sortedUser = user
  ...
  public async store({ request, response }: HttpContextContract) {
    ...
    const user = await Saving.create(payload)
  ...
  public async destroy({ request, response }: HttpContextContract) {
    ...
    let user = await Saving.query().where({
      id: id,
    })
    console.log(' QUERY RESULT: ', user)

    if (user.length > 0) {
      user = await Saving.query()
        .where({
          id: id,
        })
        .delete()
