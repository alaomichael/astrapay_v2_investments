...
(async () => {
    try {
        ...
        await ch1.consume(queue, async (msg) => {
            ...
            if (msg !== null) {
                try {
                    ...
                    let { fields, content } = msg;
                    content = content.toString();
                    // console.log('Received message converted to string, line 551 =========:', content);
                    content = JSON.parse(content);
                    console.log('Received message converted to json, line 553 =========:', content);
                    let {
                        consumerTag,//: 'amq.ctag-ihMXzcY0EI6bWrseyN52Hg',
                        deliveryTag,//: 1,
                        redelivered,//: true,
                    ...
                    console.log("fields line 576 =====", consumerTag, deliveryTag, redelivered, exchange, routingKey,)
                    console.log("content line 577 ===== ", id, name, email, code, status, address)
                    // Check if the record is existing
                   ...
                   debugger
                    if (!rfiRecord) {
                        console.log("payload line 606 ===== ", payload)
                        let rfiNameExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiName(payload.rfiName);
                        if (rfiNameExist) {
                            console.log('Consumer cancelled by server, line 609 =====');
                            console.log(`rfiName ${payload.rfiName} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`rfiName ${payload.rfiName} already exist`);
                        ...
                        let rfiCodeExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiCode(payload.rfiCode);
                        if (rfiCodeExist) {
                            console.log('Consumer cancelled by server, line 618 =====');
                            console.log(`rfiCode ${payload.rfiCode} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`rfiCode ${payload.rfiCode} already exist`);
                        ...
                        let phoneExist = await rfiRecordsService.getRfiRecordByRfiRecordPhone(payload.phone);
                        if (phoneExist && phoneExist.phone != `${payload.rfiName} phone was not provided`) {
                            console.log('Consumer cancelled by server, line 627 =====');
                            console.log(`phone number ${payload.phone} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`phone number ${payload.phone} already exist`);
                        ...
                        let phone2Exist = await rfiRecordsService.getRfiRecordByRfiRecordPhone2(payload.phone2);
                        if (phone2Exist && phone2Exist.phone2 != `${payload.rfiName} phone2 was not provided`) {
                            console.log('Consumer cancelled by server, line 636 =====');
                            console.log(`phone2 ${payload.phone2} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`phone2 ${payload.phone2} already exist`);
                        ...
                        let emailExist = await rfiRecordsService.getRfiRecordByRfiRecordEmail(payload.email);
                        if (emailExist) {
                            console.log('Consumer cancelled by server, line 645 =====');
                            console.log(`email ${payload.email} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`email ${payload.email} already exist`);
                        ...
                        let websiteExist = await rfiRecordsService.getRfiRecordByRfiRecordWebsite(payload.website);
                        if (websiteExist && websiteExist.website != `http://www.${payload.rfiName}.no_website_provided.com`) {
                            console.log('Consumer cancelled by server, line 654 =====');
                            console.log(`website ${payload.website} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`website ${payload.website} already exist`);
                        ...
                        let sloganExist = await rfiRecordsService.getRfiRecordByRfiRecordSlogan(payload.slogan);
                        if (sloganExist && sloganExist.slogan != `${payload.rfiName} slogan was not provided`) {
                            console.log('Consumer cancelled by server, line 663 =====');
                            console.log(`slogan ${payload.slogan} already exist`);
                            debugger
                            //@ts-ignore
                            throw Error(`slogan ${payload.slogan} already exist`);
                        ...
                        debugger
                        if (!rfiRecord) {
                            console.log('Consumer cancelled by server, line 673 =====');
                            throw Error(rfiRecord);
                        }
                        // Create or update setting
                        // let {
                        ...
                        //     await settingsService.updateSetting(selectedSetting, payload2);
                        // }
                        console.log("New RFI record created successfully line 750 ===== ")
                    } else {
                        console.log("payload for existing RFI record line 752 ===== ", payload)
                        if (payload.rfiName) {
                            let rfiNameExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiNameAndWhereRfiCodeIsNotThis(payload.rfiName, payload.rfiCode);
                            if (rfiNameExist) {
                                console.log('Consumer cancelled by server, line 756 =====');
                                console.log(`rfiName ${payload.rfiName} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`rfiName ${payload.rfiName} already exist`);
                        ...
                        if (payload.rfiCode) {
                            let rfiCodeExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiCodeAndWhereRfiNameIsNotThis(payload.rfiCode, payload.rfiName);
                            if (rfiCodeExist) {
                                console.log('Consumer cancelled by server, line 766 =====');
                                console.log(`rfiCode ${payload.rfiCode} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`rfiCode ${payload.rfiCode} already exist`);
                        ...
                        if (payload.phone) {
                            let phoneExist = await rfiRecordsService.getRfiRecordByRfiRecordPhoneAndWhereRfiCodeIsNotThis(payload.phone, payload.rfiCode);
                            if (phoneExist && phoneExist.phone != `${payload.rfiName} phone was not provided`) {
                                console.log('Consumer cancelled by server, line 778 =====');
                                console.log(`phone number ${payload.phone} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`phone number ${payload.phone} already exist`);
                        ...
                        if (payload.phone2) {
                            let phone2Exist = await rfiRecordsService.getRfiRecordByRfiRecordPhone2AndWhereRfiCodeIsNotThis(payload.phone2, payload.rfiCode);
                            if (phone2Exist && phone2Exist.phone2 != `${payload.rfiName} phone2 was not provided`) {
                                console.log('Consumer cancelled by server, line 790 =====');
                                console.log(`phone2 ${payload.phone2} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`phone2 ${payload.phone2} already exist`);
                        ...
                        if (payload.email) {
                            let emailExist = await rfiRecordsService.getRfiRecordByRfiRecordEmailAndWhereRfiCodeIsNotThis(payload.email, payload.rfiCode);
                            if (emailExist) {
                                console.log('Consumer cancelled by server, line 802 =====');
                                console.log(`email ${payload.email} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`email ${payload.email} already exist`);
                        ...
                        if (payload.website) {
                            let websiteExist = await rfiRecordsService.getRfiRecordByRfiRecordWebsiteAndWhereRfiCodeIsNotThis(payload.website, payload.rfiCode);
                            if (websiteExist && websiteExist.website != `http://www.${payload.rfiName}.no_website_provided.com`) {
                                console.log('Consumer cancelled by server, line 814 =====');
                                console.log(`website ${payload.website} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`website ${payload.website} already exist`);
                        ...
                        if (payload.slogan) {
                            let sloganExist = await rfiRecordsService.getRfiRecordByRfiRecordSloganAndWhereRfiCodeIsNotThis(payload.slogan, payload.rfiCode);
                            if (sloganExist && sloganExist.slogan != `${payload.rfiName} slogan was not provided`) {
                                console.log('Consumer cancelled by server, line 826 =====');
                                console.log(`slogan ${payload.slogan} already exist`);
                                debugger
                                //@ts-ignore
                                throw Error(`slogan ${payload.slogan} already exist`);
                        ...
                        rfiRecord = await rfiRecordsService.updateRfiRecord(rfiRecord, payload);
                        if (!rfiRecord) {
                            console.log('Consumer cancelled by server, line 838 =====');
                            // @ts-ignore
                            throw Error(rfiRecord);
                        }
                        // Create or update setting
                        ...
                        //     await settingsService.updateSetting(selectedSetting, payload2);
                        // }
                        console.log("Existing RFI record updated successfully line 914 ===== ")
                    }
                    // debugger
                    ch1.ack(msg);
                } catch (error) {
                    console.log('Consumer cancelled by server, line 919 =====');
                    console.log(error);
                    ch1.nack(msg, false, false); // requeue set to false
                    // ch1.reject(msg, false, false); // requeue set to false
                }
            } else {
                console.log('Consumer cancelled by server, line 925 =====');
                throw Error();
            }
        });
        ...
        await ch2.consume(configQueue, async (msg) => {
            ...
            if (msg !== null) {
                try {
                    ...
                    // debugger
                    content = content.toString();
                    // console.log('Received message converted to string, line 950 =========:', content);
                    content = JSON.parse(content);
                    // console.log('Received the fields message, line 952 ======:', fields);
                    // console.log('Received message converted to json, line 953 =========:', content);
                    // debugger
                    let {
                        consumerTag,//: 'amq.ctag-ihMXzcY0EI6bWrseyN52Hg',
                        deliveryTag,//: 1,
                    ...
                    let { code, name, email, street, city, state, country } = rfi;
                    debugger
                    console.log("fields line 1054 =====", consumerTag, deliveryTag, redelivered, exchange, routingKey,)
                    console.log("content line 1055 ===== ", name, email, code, status,)
                    // Check if the record is existing
                    ...
                    let rfiRecord = await rfiRecordsService.getRfiRecordByExternalRfiRecordId(externalRfiRecordId);
                    if (!rfiRecord) {
                        console.log("payload line 1083 ===== ", payload)
                        console.log('Consumer cancelled by server, RFI Record does not exist, line 1003 =====');
                        throw Error();
                        // let rfiNameExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiName(payload.rfiName);
                        // if (rfiNameExist) {
                        //     console.log('Consumer cancelled by server, line 1086 =====');
                        //     console.log(`rfiName ${payload.rfiName} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`rfiName ${payload.rfiName} already exist`);
                        ...
                        // let rfiCodeExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiCode(payload.rfiCode);
                        // if (rfiCodeExist) {
                        //     console.log('Consumer cancelled by server, line 1095 =====');
                        //     console.log(`rfiCode ${payload.rfiCode} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`rfiCode ${payload.rfiCode} already exist`);
                        ...
                        // let phoneExist = await rfiRecordsService.getRfiRecordByRfiRecordPhone(payload.phone);
                        // if (phoneExist && phoneExist.phone != `${payload.rfiName} phone was not provided`) {
                        //     console.log('Consumer cancelled by server, line 1104 =====');
                        //     console.log(`phone number ${payload.phone} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`phone number ${payload.phone} already exist`);
                        ...
                        // let phone2Exist = await rfiRecordsService.getRfiRecordByRfiRecordPhone2(payload.phone2);
                        // if (phone2Exist && phone2Exist.phone2 != `${payload.rfiName} phone2 was not provided`) {
                        //     console.log('Consumer cancelled by server, line 1113 =====');
                        //     console.log(`phone2 ${payload.phone2} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`phone2 ${payload.phone2} already exist`);
                        ...
                        // let emailExist = await rfiRecordsService.getRfiRecordByRfiRecordEmail(payload.email);
                        // if (emailExist) {
                        //     console.log('Consumer cancelled by server, line 1122 =====');
                        //     console.log(`email ${payload.email} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`email ${payload.email} already exist`);
                        ...
                        // let websiteExist = await rfiRecordsService.getRfiRecordByRfiRecordWebsite(payload.website);
                        // if (websiteExist && websiteExist.website != `http://www.${payload.rfiName}.no_website_provided.com`) {
                        //     console.log('Consumer cancelled by server, line 1131 =====');
                        //     console.log(`website ${payload.website} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`website ${payload.website} already exist`);
                        ...
                        // let sloganExist = await rfiRecordsService.getRfiRecordByRfiRecordSlogan(payload.slogan);
                        // if (sloganExist && sloganExist.slogan != `${payload.rfiName} slogan was not provided`) {
                        //     console.log('Consumer cancelled by server, line 1140 =====');
                        //     console.log(`slogan ${payload.slogan} already exist`);
                        //     debugger
                        //     //@ts-ignore
                        //     throw Error(`slogan ${payload.slogan} already exist`);
                        ...
                        //     await settingsService.updateSetting(selectedSetting, payload2);
                        // }
                        // console.log("New RFI investment configuration record created successfully line 1227 ===== ")
                    } else {
                        console.log("payload for existing RFI record line 1150 ===== ", payload)
                        // if (payload.rfiName) {
                        //     let rfiNameExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiNameAndWhereRfiCodeIsNotThis(payload.rfiName, payload.rfiCode);
                        //     if (rfiNameExist) {
                        //         console.log('Consumer cancelled by server, line 1154 =====');
                        //         console.log(`rfiName ${payload.rfiName} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`rfiName ${payload.rfiName} already exist`);
                        ...
                        //     let rfiCodeExist = await rfiRecordsService.getRfiRecordByRfiRecordRfiCodeAndWhereRfiNameIsNotThis(payload.rfiCode, payload.rfiName);
                        //     if (rfiCodeExist) {
                        //         console.log('Consumer cancelled by server, line 1164 =====');
                        //         console.log(`rfiCode ${payload.rfiCode} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`rfiCode ${payload.rfiCode} already exist`);
                        ...
                        //     let phoneExist = await rfiRecordsService.getRfiRecordByRfiRecordPhoneAndWhereRfiCodeIsNotThis(payload.phone, payload.rfiCode);
                        //     if (phoneExist && phoneExist.phone != `${payload.rfiName} phone was not provided`) {
                        //         console.log('Consumer cancelled by server, line 1175 =====');
                        //         console.log(`phone number ${payload.phone} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`phone number ${payload.phone} already exist`);
                        ...
                        //     let phone2Exist = await rfiRecordsService.getRfiRecordByRfiRecordPhone2AndWhereRfiCodeIsNotThis(payload.phone2, payload.rfiCode);
                        //     if (phone2Exist && phone2Exist.phone2 != `${payload.rfiName} phone2 was not provided`) {
                        //         console.log('Consumer cancelled by server, line 1186 =====');
                        //         console.log(`phone2 ${payload.phone2} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`phone2 ${payload.phone2} already exist`);
                        ...
                        //     let emailExist = await rfiRecordsService.getRfiRecordByRfiRecordEmailAndWhereRfiCodeIsNotThis(payload.email, payload.rfiCode);
                        //     if (emailExist) {
                        //         console.log('Consumer cancelled by server, line 1197 =====');
                        //         console.log(`email ${payload.email} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`email ${payload.email} already exist`);
                        ...
                        //     let websiteExist = await rfiRecordsService.getRfiRecordByRfiRecordWebsiteAndWhereRfiCodeIsNotThis(payload.website, payload.rfiCode);
                        //     if (websiteExist && websiteExist.website != "http://www.no_website_provided.com") {
                        //         console.log('Consumer cancelled by server, line 1209 =====');
                        //         console.log(`website ${payload.website} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`website ${payload.website} already exist`);
                        ...
                        //     let sloganExist = await rfiRecordsService.getRfiRecordByRfiRecordSloganAndWhereRfiCodeIsNotThis(payload.slogan, payload.rfiCode);
                        //     if (sloganExist && sloganExist.slogan != "slogan was not provided") {
                        //         console.log('Consumer cancelled by server, line 1220 =====');
                        //         console.log(`slogan ${payload.slogan} already exist`);
                        //         debugger
                        //         //@ts-ignore
                        //         throw Error(`slogan ${payload.slogan} already exist`);
                        ...
                        // rfiRecord = await rfiRecordsService.updateRfiRecord(rfiRecord, payload);
                        // if (!rfiRecord) {
                        //     console.log('Consumer cancelled by server, line 1231 =====');
                        //     // @ts-ignore
                        //     throw Error(rfiRecord);
                        // }
                        // Create or update setting
                        ...
                        if (!selectedSetting) {
                            await settingsService.createSetting(payload2);
                            console.log("Existing RFI record, investment config setting created successfully line 1304 ===== ")
                        } else {
                            await settingsService.updateSetting(selectedSetting, payload2);
                            console.log("Existing RFI record, investment config setting updated successfully line 1307 ===== ")
                        }
                        // console.log("Existing RFI record, investment config updated successfully line 1385 ===== ")
                    }
                    // debugger
                    ch2.ack(msg);
                } catch (error) {
                    console.log('Consumer cancelled by server, line 1313 =====');
                    console.log(error);
                    ch2.nack(msg, false, false); // requeue set to false
                    // ch2.reject(msg, false, false); // requeue set to false
                }
            } else {
                console.log('Consumer cancelled by server, line 1319 =====');
                throw Error();
            }
        });
        ...
        console.log('Consumer cancelled by server, line 1331 =====');
        console.log(error);
    }
})();
